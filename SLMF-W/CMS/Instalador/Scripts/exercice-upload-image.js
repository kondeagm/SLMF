$(function () {
    var dropbox = $('#dropbox'),
        ...
        fileIndex = 0;
    var id_image_file = $("#idejercicio").val();

    dropbox.filedrop({
        paramname: 'file',
        maxfiles: 1,
        maxfilesize: 1,
        url: '/Ejercicio/UploadImage/' + id_image_file,
        uploadFinished: function (i, file, response) {
            ...
            if (json.estado > 0) {
                ...
                document.getElementById('main-container-drop-thumb').style.display = 'inherit';
            }
            else {
                $('#image-uploaded').show();
        ...
        uploadStarted: function (i, file) {
            document.getElementById('main-container-drop-thumb').style.display = 'none';
            $('#buttonback').hide();
            $('#waitter').show();
            fileIndex++;
            createImage(file);
