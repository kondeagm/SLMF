...
namespace SLMFCMS.Controllers
{
    ...
    public class AlimentoController : Controller
    {
        ...
        public ActionResult Index(string sSortOrder, string sCurrentFilter, string sSearchString, int? pagina)
        {
            ...
            var rowsAlimento = from s in db.Alimento.Where(p => p.Suplemento == false && p.Preparado == false).Include(p => p.Nutriente)
                               select s;
        ...
        public ActionResult Details(int? id, int? pagina)
        {
            ...
            Alimento bdAlimento = db.Alimento.Where(p => p.ID == id && p.Suplemento == false && p.Preparado == false).First();
            if (bdAlimento == null)
            {
                return HttpNotFound();
            }
        ...
        public ActionResult Create(int? pagina, [Bind(Include = "ID,Nombre,NutrienteID,Suplemento,Preparado")] Alimento alimento)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    ModelState.AddModelError("Nombre", "Ya existe un Alimento con este Nombre");
                }
                else
                {
                    db.Alimento.Add(alimento);
                    db.SaveChanges();
                    return RedirectToAction("Index", "Alimento", new { sSearchString = alimento.Nombre });
                }
            }
        ...
        public ActionResult DeleteConfirmed(int id, int? pagina)
        {
            ...
            return RedirectToAction("Index", "Alimento", new { pagina = ViewBag.Pagina });
        }
